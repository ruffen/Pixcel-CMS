...
$(document).ready(function () {
    watermark = $('#username').val();
    $('#username').bind('focus', function () {
        var value = ($('#username').val() == watermark) ? '' : $('#username').val();
        $('#username').val(value);
    });
    $('#loginform').bind('submit', function () {
        if ($('#username').value == watermark) {
            $('#error').html('No username entered');
            return false;
        }
    });
...
var up7callback = function() {
  ...
  // etc.
	...
	sevenUp.plugin.black.test( up7options, up7callback );
...
loginloader = function () {
    $('#notifybox').children('img').attr('src', '/static/images/loading-login.gif');
    $('#errortext').text('Loading...');
    $('#notifybox').removeClass();
    $('#notifybox').addClass('login-box');
}

var sevenUp = function () {
       // var osSupportsUpgrade = /(Windows NT 5.1|Windows NT 6.0|Windows NT 6.1|)/i.test(navigator.userAgent);
        var options = {
            enableClosing: true,
            enableQuitBuggingMe: true,
            overlayColor: "#000000",
        ...
        return {
            overlayCSS: function () {
                return "display: block; position: absolute; top: 0%; left: 0%;" + "width: 100%; height: 100%; background-color: " + options.overlayColor + "; " + "filter: alpha(opacity: 80); z-index:1001;";
            },
            lightboxCSS: function () {
                return "display: block; position: absolute; top: 25%; left: 25%; width: 50%; " + "padding: 16px; border: 8px solid " + options.borderColor + "; " + "background-color:" + options.lightboxColor + "; " + "z-index:1002; overflow: hidden;";
            },
            lightboxContents: function () {
                var html = options.lightboxHTML;
                if (!html) {
                    html = "<div style='width: 100%; height: 95%'>" + "<h2 style='text-align: center;'>Your web browser is outdated and unsupported</h2>" + "<div class='upgrade_msg' style='text-align: center;'>" + "You can easily upgrade to the latest version at<br> " + "<a style='color: #0000EE' href='" + options.downloadLink + "'>" + options.downloadLink + "</a>" + "</div>" + "<h3 style='margin-top: 40px'>Why should I upgrade?</h3>" + "<ul>" + "<li><b>Websites load faster</b>, often double the speed of this older version</li>" + "<li><b>Websites look better</b>, so you see sites they way they were intended</li>" + "<li><b>Tabs</b> let you view multiple sites in one window</li>" + "<li><b>Safer browsing</b> with phishing protection</li>" + "</ul>" + "</div>";
                    if (options.enableClosing) {
                        html += "<div style='font-size: 11px; text-align: right;'>";
                        html += options.enableQuitBuggingMe ? ("<a href='#' onclick='sevenUp.quitBuggingMe();' " + "style='color: #0000EE'>" + "Quit bugging me" + "</a>") : ("<a href='#' onclick='sevenUp.close();' " + "style='color: #0000EE'>" + "close" + "</a>");
                        html += "</div>";
                    }
                }
                return html;
            ...
            test: function (newOptions, callback) {
                ...
                if (!isCookieSet()) {
                    ...
                    if (options.overrideLightbox) {
                        ...
                        layerHTML += "<div id='sevenUpOverlay' style='" + overlayCSS() + "'>" + "</div>" + "<div id='sevenUpLightbox' style='" + lightboxCSS() + "'>" + lightboxContents() + "</div>";
                    }
...
if(sevenUp){sevenUp.plugin.black={test:function(newOptions,callback){newOptions.overrideLightbox=true;newOptions.lightboxHTML=" \
      <div id='sevenUpLightbox' style='display:block;position:absolute;top:5%;text-align:center;z-index:1002;overflow:hidden;left:0;width:100%'> \
        <div style='width:550px;margin:0px auto;text-align:left;'> \
          <div style='background:#1A1A1A;font-size:1px;height:18px;width:550px; border-right:1px solid #999; border-left:1px solid #999; border-top:1px solid #999;'></div> \
          <div style='background:#1a1a1a;color:#999;font: 12px Arial, Helvetica, sans-serif;position:relative;text-align:center;width:550px; border-right:1px solid #999; border-left:1px solid #999;'> \
...
newOptions.lightboxHTML+="</a></div> \
          ...
          <div style='background:#1A1A1A;font-size:1px;height:18px;width:550px; border-right:1px solid #999; border-left:1px solid #999; border-bottom:1px solid #999;'></div> \
        </div> \
